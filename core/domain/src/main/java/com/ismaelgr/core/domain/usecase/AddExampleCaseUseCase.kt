...
class AddExampleCaseUseCase @Inject constructor(private val caseRepository: CaseRepository) {
    ...
    operator fun invoke(userid: UUID): Flow<Unit> = flow {
        ...
        val newCase = Case(id = UUID.randomUUID(), material = "Material ${currentCases.size + 1}", ownerId = userid, date = Calendar.getInstance().formatToString(), type = "Case type x", state = CaseState.entries.random())
